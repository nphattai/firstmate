...
3. **Write within the existing boundaries.**
   ...
   - For each durable captain decision or learning this sweep files to a memory file, also push it to this home's fleet memory with `bin/fm-remember.sh "<entry text>"`, so recall accumulates it next spawn (memval-04).
     This is a best-effort side-effect that never changes what the pass files to disk: the helper does nothing when this home has no memory wiring, and its write path dedups, so overlapping with a decision that captain-hold already wrote is a safe no-op.
